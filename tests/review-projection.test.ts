...
async function person(tag: string, workroomId: string | null, email: string): Promise<ClientActor> {
 ...
 */
async function stage(tag = `P${seq++}`): Promise<Stage> {
...
async function round(s: Stage): Promise<string> {
...
test("staff have no way to open a feedback item, and clients none to close a round", () => {
  ...
  assert.ok(!studio.includes("editReviewNote"), "a studio action can edit somebody's words");
  assert.ok(!studio.includes("removeReviewNote"), "a studio action can remove somebody's words");
